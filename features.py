...
def scores():
    ...
    for i in range(1, len(table_sibling)):
        # tr
        for tr_data in table_sibling[i].findAll(
            "a", {"name": "&lpos=nba:schedule:score"}
        ):
            # print (tr_data.contents[0].replace(",", " -"))
            score = []
            for tr_player_data in table_sibling[i].findAll(
                "a", {"name": "&lpos=nba:schedule:player"}
            ):
            ...
            data.append(
                [tr_data.contents[0].replace(",", " -"), score[0] + " & " + score[1]]
            )
    ...
    if data != [["SCORE", "Winner & Loser High"]]:
        ...
        content = click.style("No matches played!!", bg="green")
        beautify_print(content)
...
def standings():
    ...
    for conf in conference:
        ...
        stand = conf.next_sibling.contents[0].table.findAll(
            "tr", {"class": "Table2__tr"}
        )
